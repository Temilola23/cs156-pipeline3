...
\caption{Six $\mathcal{W}$-space draws from the trained StyleGAN3-ADA generator, upscaled from $512 \times 512$ to $1920 \times 1080$, drawn directly from \texttt{\_local\_assembly/stylegan3\_upscaled/}. The six panels (\texttt{trailer\_scene\_\{09, 11, 20, 26, 27, 31\}}) correspond to the six cluster centres returned by $K$-means with $K=6$ on a pool of 5000 candidate $\mathbf{w}$ vectors. They span the four colour clusters (cool-grey, warm-orange, noir-teal, monochrome) and the two motion signatures (static landscape, directional streak) that the ADA-regularised discriminator converged to, which is the evidence that adaptive discriminator augmentation prevented the palette collapse documented in Appendix~\ref{sec:challenges}. The production video uses $\mathbf{w}$-space interpolation $\mathbf{w}_t = (1-t)\mathbf{w}_a + t\mathbf{w}_b$ between consecutive cluster centres to produce the interstitial transitions; the smoothness of those transitions is the empirical consequence of the alias-free generator's shift-equivariance property derived below.}
...
\caption{Six raw StyleGAN3-ADA unconditional W-space samples upscaled from $512 \times 512$ to $1920 \times 1080$, drawn directly from \texttt{\_local\_assembly/stylegan3\_upscaled/} after 20~kimg of training on the 20{,}000-frame trailer corpus. Panels (a)--(f) correspond to \texttt{trailer\_scene\_\{09,11,20,26,27,31\}}. Panel (a) is a blue-grey cloud / mountain composition; the cool monochrome and low-horizon layout match the misty Arrakis establishing shots of \emph{Dune} and the ice-sheet openers of \emph{Interstellar}. Panel (b) is an orange-sky heat haze; the sodium-orange sky over a dark silhouette mimics the Tatooine twin-sun horizons in \emph{Star Wars} and the desert sunrise that opens \emph{Blade Runner 2049}. Panel (c) is a dark-alley reflection with green-teal highlights on wet asphalt; the vocabulary is the noir-Gotham alleyway that recurs throughout \emph{The Dark Knight}. Panel (d) is an industrial-sunset silhouette, a wide low-horizon smokestack landscape colour-graded toward the gold-purple tail of a coal-country sunset; this is the \emph{Interstellar} dust-bowl palette. Panel (e) is a blue-green streak, a vertically banded high-motion sample where the convolutional filters have rendered energy-field streaks; the signature matches the tractor-beam and lightsaber clash cinematography in \emph{The Force Awakens}. Panel (f) is a monochrome reflection on wet pavement with a centred upright form; the low-saturation rain-slick geometry echoes the Gotham rainy-street language that \emph{The Batman} (2022) inherited from Nolan. The six panels together cover four colour clusters (cool-grey, warm-orange, noir-teal, monochrome) and two motion signatures (static landscape, directional streak), which is the evidence that adaptive discriminator augmentation prevented the palette collapse documented in Appendix~\ref{sec:challenges}.}
