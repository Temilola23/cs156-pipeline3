...
\subsection*{Reproducibility, tests, and code documentation}
\addcontentsline{toc}{subsection}{Reproducibility, tests, and code documentation}

Every non-trivial source module under \texttt{src/} ships with a matching unit-test file under \texttt{tests/} (for example \texttt{tests/test\_hierarchical\_anova.py} covers \texttt{src/hierarchical\_anova.py}, \texttt{tests/test\_conformal.py} covers the split-conformal quantile logic, and similar pairs exist for the GP posterior, Kalman ladder, particle filter, HMM forward-backward, LightGCN message-passing, HAN attention, GenMatch fitness, the TVAE ELBO, IPW/AIPW estimators, MC-Dropout BNN, and Thompson bandit). The tests are run with \texttt{pytest} from the repository root and assert numerical properties that the derivations in \texttt{derivations/} require: conformal intervals achieve at least the nominal floor on held-out calibration folds, the Kalman gain matches the closed-form solution on a Gaussian toy problem, the HMM forward pass agrees with the brute-force enumeration on a small regime grid, and so on. They are not end-to-end integration tests against the real 324-rating dataset (that is what the \texttt{scripts/} directory does); they are correctness tests on synthetic inputs with known ground truth, designed to catch regressions in the primitives. In parallel, every function and method in \texttt{src/} and every training script in \texttt{scripts/} carries a PEP~257-style docstring that states the function's purpose, its parameters with types, its return type, and, where relevant, the shape of the returned array, following the \texttt{numpydoc} / Google-style conventions. The main submission notebook \texttt{notebooks/main\_submission.ipynb} is annotated in the same style: each code cell is preceded by a markdown cell that explains what the cell does and why, and function definitions inside code cells carry inline docstrings with \texttt{Args:}, \texttt{Returns:}, and \texttt{Raises:} blocks where applicable. The intention is that a reader who opens any file in the repository should be able to understand both the local contract (from the docstring) and the global role (from the paper section that cites it) without having to read the surrounding code.
...
\caption{Pedagogical verification of the forward diffusion schedule and noising progression (cosine schedule, toy latent, oracle denoiser; production SDXL uses the linear schedule discussed above). Left panel: the cosine schedule $\bar\alpha_t = \cos^2(((t/T + s)/(1+s)) \cdot \pi/2)$ of Nichol and Dhariwal (2021), normalised so $\bar\alpha_0 = 1$. The curve decays from 1 at $t=0$ to near zero at $t=T=1000$, which is precisely the property exploited by Equation~\ref{eq:diff-forward}: the forward process drives the latent to an isotropic Gaussian that the reverse process can initialise from. The four annotated diffusion times ($t \in \{0, 250, 500, 750, 1000\}$) mark the slices rendered in the right panel. Right panel: samples from the forward marginal $q(\mathbf{z}_t \mid \mathbf{z}_0) = \mathcal{N}(\sqrt{\bar\alpha_t}\,\mathbf{z}_0,\,(1-\bar\alpha_t)\,\mathbf{I})$ at the five annotated timesteps, stacked so the signal-to-noise ratio is visually comparable. At $t=0$ the signal is the clean latent; by $t=500$ the signal is roughly balanced with injected Gaussian noise ($\bar\alpha_{500} \approx 0.25$); at $t=1000$ the latent is indistinguishable from a sample of $\mathcal{N}(\mathbf{0}, \mathbf{I})$. The cosine schedule is used here for the notebook sanity check (cell~77) because it admits a closed-form $\bar\alpha_t$ and decays smoothly away from $t=0$; production SDXL inherits the pretrained linear schedule ($\beta_1 = 10^{-4}$, $\beta_T = 0.02$) unchanged, for the reasons given in Section~\ref{sec:diffusion}. The figure therefore visualises the shape of the noising trajectory that the U-Net is trained to reverse, rather than the specific $\bar\alpha_t$ values SDXL consumes at inference.}
